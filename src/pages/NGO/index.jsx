...
function NGODashboardPage() {
  ...
  const [signedEvidenceUrl, setSignedEvidenceUrl] = useState(null)
  ...
  const fetchCases = async () => {
    ...
    else console.error('Fetch cases error:', error)
    setLoading(false)
  }
  ...
  const openCase = async (c) => {
    ...
    if (c.evidence_file_url) {
      const parts = c.evidence_file_url.split('/evidence/')
      const fileName = parts[1]
      if (fileName) {
        const { data: urlData, error: urlError } = await supabase.storage
          .from('evidence')
          .createSignedUrl(decodeURIComponent(fileName), 3600)
        if (urlError) console.error('Signed URL error:', urlError)
        setSignedEvidenceUrl(urlData?.signedUrl || null)
      }
    }
  }
  ...
  const insertLog = async (caseId, oldStatus, newStatus, noteText) => {
    ...
    console.log('Inserting log:', payload)
    const { data, error } = await supabase
      .from('case_status_logs')
      .insert([payload])
      .select()
  ...
  const handleAcceptCase = async () => {
    ...
    try {
      ...
      await insertLog(
        ...
        selectedCase.status,
        'assigned',
        `Case accepted by ${ngoName}.`
      )
      await refreshDetail(selectedCase.case_id)
    } catch (err) {
      setUpdateError(`Failed to accept case: ${err.message}`)
    }
    setUpdating(false)
  }
  ...
  const handleDeclineCase = async () => {
    ...
    try {
      ...
      setUpdateError(`Failed to decline: ${err.message}`)
    }
    setUpdating(false)
  }
            ...
            <div className="flex items-center justify-between py-3.5">
              ...
              <div className="flex items-center gap-2">
                ...
                <a href="https://google.com" target="_blank" rel="noopener noreferrer"
                  className="text-xs bg-red-600 text-white px-3 py-2 rounded-lg font-bold">
                 Exit
                </a>
              </div>
            </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 md:p-6">
            <div className="flex items-start justify-between flex-wrap gap-4 mb-5">
              <div>
                ...
                <p className="font-mono font-extrabold text-gray-900 text-lg tracking-wider">
                  {selectedCase.case_id}
                </p>
                <p className="text-xs text-gray-400 mt-1">
                  Submitted {new Date(selectedCase.created_at).toLocaleDateString('en-NG', {
                    day: 'numeric', month: 'long', year: 'numeric',
                  })}
                </p>
              </div>
              <div className="flex items-center gap-2 flex-wrap">
                <span className={`text-sm font-bold px-4 py-2 rounded-full border ${s.color}`}>
                  {s.label}
                </span>
                {isAccepted && (
                  <span className="text-xs bg-emerald-100 text-emerald-700 px-3 py-1.5 rounded-full font-bold border border-emerald-200">
                    ✓ Accepted by your NGO
                  </span>
                )}
                {isDeclined && (
                  <span className="text-xs bg-red-100 text-red-700 px-3 py-1.5 rounded-full font-bold border border-red-200">
                    ✗ Declined
                  </span>
                )}
              </div>
            ...
            <div className="mb-5">
              ...
              <div className="flex items-center gap-0">
                {STATUS_STEPS.map((step, i) => (
                  <div key={step} className="flex items-center flex-1">
                    <div className={`h-1.5 flex-1 rounded-full transition-all ${
                      i <= currentStepIndex ? 'bg-purple-600' : 'bg-gray-100'
                    }`} />
                    {i < STATUS_STEPS.length - 1 && (
                      <div className={`w-2.5 h-2.5 rounded-full flex-shrink-0 ${
                        i < currentStepIndex ? 'bg-purple-600'
                        : i === currentStepIndex ? 'bg-purple-600 ring-2 ring-purple-200'
                        : 'bg-gray-200'
                      }`} />
                    )}
                  </div>
                ...
                <div className={`w-2.5 h-2.5 rounded-full flex-shrink-0 ${
                  currentStepIndex === STATUS_STEPS.length - 1 ? 'bg-emerald-600' : 'bg-gray-200'
                }`} />
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
              {[
                ...
                { label: 'Area / LGA', value: selectedCase.lga || 'Not specified' },
                {
                  ...
                  value: selectedCase.incident_date
                    ? new Date(selectedCase.incident_date).toLocaleDateString('en-NG', {
                        day: 'numeric', month: 'long', year: 'numeric',
                      })
                    : 'Not specified',
                },
                {
                  label: 'Contact',
                  value: selectedCase.contact_email || 'Anonymous — no email',
                  warn: !selectedCase.contact_email,
                },
                {
                  label: 'Evidence Photo',
                  value: selectedCase.evidence_file_url ? 'Photo attached — see below' : 'None provided',
                },
              ].map((item) => (
                <div key={item.label}
                  className={`rounded-xl p-3 border ${
                    item.warn ? 'bg-amber-50 border-amber-100' : 'bg-gray-50 border-gray-100'
                  }`}>
                  <p className="text-xs text-gray-400 mb-0.5">{item.label}</p>
                  <p className={`text-sm font-semibold ${item.warn ? 'text-amber-700' : 'text-gray-800'}`}>
                    {item.value}
                  </p>
                </div>
              ))}
          ...
          {!selectedCase.contact_email && (
            <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4">
              ...
              <p className="text-xs text-amber-700 leading-relaxed">
                They did not provide an email, this was intentional. You cannot contact them directly.
                Any note you save below will be visible to them when they check their Case ID on the tracking page.
                Use this to share your office address, a safe phone number, or helpful resources.
              </p>
            </div>
          )}
          ...
          {isPending && selectedCase.status !== 'resolved' && (
            <div className="bg-white rounded-2xl border border-purple-200 shadow-sm p-5">
              ...
              <p className="text-sm text-gray-600 leading-relaxed mb-4">
                Review the case details above. If your organisation can support this survivor, accept the case.
                If you cannot, decline it with a reason. The admin will be notified so another arrangement can be made.
              </p>
              <div className="flex flex-col sm:flex-row gap-3">
                <button onClick={handleAcceptCase} disabled={updating}
                  className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-sm disabled:opacity-50 transition-all shadow-sm">
                  <ThumbsUp className="w-4 h-4" /> Accept This Case
                </button>
                <button onClick={() => setShowDeclineModal(true)}
                  className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl bg-white hover:bg-red-50 text-red-700 font-bold text-sm border border-red-200 hover:border-red-300 transition-all">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 md:p-6">
              ...
              {signedEvidenceUrl ? (
                ...
                <div className="bg-gray-50 rounded-xl border border-gray-100 p-8 text-center">
                  ...
                  <p className="text-xs text-gray-400">Loading evidence photo...</p>
                </div>
              )}
            </div>
          )}
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 md:p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-xs text-gray-400">
                {caseLogs.length} entr{caseLogs.length !== 1 ? 'ies' : 'y'}
              </span>
            </div>
            {caseLogs.length === 0 ? (
              <p className="text-xs text-gray-400 text-center py-4">
                No actions recorded yet. Document your first action below.
              </p>
            ) : (
                  ...
                  return (
                    <div key={i} className="flex items-start gap-3 py-3 first:pt-0 last:pb-0">
                      <div className={`w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 ${
                        isStatusChange ? 'bg-purple-700' : 'bg-gray-100'
                      }`}>
                        {isStatusChange
                        ...
                        <div className="flex items-center gap-2 flex-wrap mb-1">
                          {isStatusChange ? (
                            <span className="text-xs font-bold text-gray-700">
                              Moved to{' '}
                              <span className={`px-1.5 py-0.5 rounded border text-xs ${STATUS_LABELS[log.new_status]?.color}`}>
                                {STATUS_LABELS[log.new_status]?.label}
                              </span>
                            </span>
                          ) : (
          ...
          <div className="bg-white rounded-2xl border border-purple-100 shadow-sm p-5 md:p-6">
              ...
              {nextStatus && selectedCase.status !== 'resolved' && (
                <button
                  ...
                  {updating ? (
                    ...
                    <>
                      Move to {STATUS_LABELS[nextStatus]?.label}
            ...
            {selectedCase.status === 'resolved' && (
              <div className="mt-4 p-4 bg-emerald-50 border border-emerald-100 rounded-xl text-center">
                ...
                <p className="text-xs text-emerald-700">You can still add notes to the history above.</p>
              </div>
            )}
          </div>
            ...
            {deletionSuccess ? (
              <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-4 text-center">
                ...
                <p className="text-xs text-emerald-700 mt-1">
                  Admin will review and process this request. All case data will be permanently deleted.
                </p>
              </div>
            ) : showDeletionForm ? (
              <div>
                <p className="text-xs text-gray-500 leading-relaxed mb-3">
                  Please confirm the victim's reason for wanting their report deleted.
                  Admin will review and permanently remove all data associated with this case.
                </p>
                <textarea
                  value={deletionReason}
                  onChange={(e) => setDeletionReason(e.target.value)}
                  rows={3}
                  placeholder="Reason the victim wants their report deleted..."
              ...
              <div>
                <p className="text-xs text-gray-500 leading-relaxed mb-3">
                  If the victim has requested deletion of their report, submit a request here.
                  Admin will permanently delete all data associated with this case.
                  If the victim provided an email, they will be notified when deletion is complete.
                </p>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center px-4"
            ...
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md p-6">
              ...
              <p className="text-sm text-gray-600 leading-relaxed mb-5">
                Please provide a reason. The admin will be notified so another arrangement
                can be made for this survivor.
              </p>
              <label className="block text-sm font-semibold text-gray-800 mb-2">
              ...
              <textarea
                ...
                rows={3}
                placeholder="e.g. Outside our specialisation, at capacity, geographical constraints..."
      ...
      <div className="max-w-5xl mx-auto px-4 md:px-8 py-8">
        ...
        <div className="mb-6">
          ...
          <p className="text-sm text-gray-400">
            Reports from{' '}
            <span className="font-semibold text-gray-700">{ngoState || '—'}</span>
          </p>
          {(!ngoState || ngoState === 'undefined') && (
            <div className="mt-3 bg-red-50 border border-red-100 rounded-xl px-4 py-3 flex items-center justify-between gap-4">
              <p className="text-xs text-red-700 font-semibold">
                Session error, state not found. Please log out and back in.
              </p>
              <button
                onClick={() => { sessionStorage.clear(); navigate('/ngo/login') }}
                className="text-xs bg-red-600 text-white px-3 py-1.5 rounded-lg font-bold flex-shrink-0">
                Log out
              </button>
            </div>
          )}
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
          {[
            ...
            { key: 'under_review', label: 'In Review', icon: <Clock className="w-4 h-4" />, color: 'text-amber-700', bg: 'bg-amber-50', border: 'border-amber-100' },
            { key: 'in_progress', label: 'In Progress', icon: <FileText className="w-4 h-4" />, color: 'text-violet-700', bg: 'bg-violet-50', border: 'border-violet-100' },
            { key: 'resolved', label: 'Resolved', icon: <CheckCircle className="w-4 h-4" />, color: 'text-emerald-700', bg: 'bg-emerald-50', border: 'border-emerald-100' },
          ].map((s) => (
            <button key={s.key} onClick={() => setFilter(s.key)}
              className={`${s.bg} border ${s.border} rounded-2xl p-4 text-left hover:shadow-md transition-all ${
                filter === s.key ? 'ring-2 ring-purple-400 ring-offset-1' : ''
              }`}>
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-2 flex-wrap">
                        ...
                        <span className={`text-xs font-bold px-2.5 py-0.5 rounded-full border ${s.color}`}>
                          {s.label}
                        </span>
                        {c.evidence_file_url && (
                    ...
                    <div className="flex items-center gap-1.5 text-xs text-purple-700 font-bold bg-purple-50 group-hover:bg-purple-100 px-3 py-2 rounded-lg border border-purple-100 transition-colors flex-shrink-0">
